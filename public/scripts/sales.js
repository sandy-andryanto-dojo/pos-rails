$(document).ready(function() {
    ...
    var myTemplateSelection = function(data) {
        return data.sku + " - " + data.text + "<input type='hidden' id='item-" + data.id + "' data-price='" + data.price + "' data-stock='" + data.stock + "'  value=''/>";
    }
    ...
    var addRow = function(id) {
        ...
        html += "<td><input name='stock[]' type='number' data-id='" + id + "' step='any' value='0' min='0' class='form-control stock' readonly='readonly' /></td>";
    ...
    $("body").on("change", ".product_id", function(e) {
        ...
        let stock = parseFloat($("#item-" + product_id).attr("data-stock"));
        ...
        if (parseInt(exists) > 1) {
            ...
            $(".stock[data-id='" + rowId + "']").val(stock);
            $(".price[data-id='" + rowId + "']").val(price);
            $(".qty[data-id='" + rowId + "']").attr("max", stock);
    ...
    $("#form-invoice").submit(function(e) {
        ...
        if (!customer_id) {
            ...
            toastShow({
                "title": "Warning",
                "message": "Petty cash must more than " + grandtotal + " !!",
                "mode": "warning"
            });
        } else {
            let form = this;
